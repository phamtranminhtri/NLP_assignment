...
The training notebooks was ran on Google Colab. Each notebook ran for about 1-2 hours on T4 free tier GPU (16GB VRAM). The code use PyTorch and PyTorch Lightning for logic, which let us configure many training options easily, such as logging, early stopping, optimizer, training step, etc.
...
For training, we use Pytorch Lightning \cite{pytorch-lightning-docs}, a library that provide us \texttt{LightningModule} to orginize the model and training step logic, and \texttt{Trainer} API for reducing boilerplate training code.
...
The configs for SBERT is quite similar to BERT, with the main difference being the hidden size, which is 384 instead of 768. We are using the most downloaded SBERT model from Hugging Face, and it's a tiny version \cite{allminilm-l6-v2}, way smaller than BERT.
...
All that's left is to evaluate the model by taking cosine similarity of the 2 embedding and make prediction. We choose a threshold of 0.7715 for maximum f1-score, based on the official SBERT documentation \cite{sentence-transformers-evaluation}.
